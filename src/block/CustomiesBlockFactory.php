...
final class CustomiesBlockFactory {
	 ...
	 */
	public function registerBlock(string $className, string $identifier, string $name, BlockBreakInfo $breakInfo, ?Model $model = null, ?CreativeInventoryInfo $creativeTabInfo = null): void {
		...
		if ($creativeTabInfo !== null) {
			$components->setTag("minecraft:creative_category", CompoundTag::create()
				->setString("category", $creativeTabInfo->getCategory() ?? CreativeInventoryInfo::CATEGORY_ALL)
				->setString("group", $creativeTabInfo->getGroup() ?? CreativeInventoryInfo::GROUP_STONE));
		}
